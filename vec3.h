...
inline vec3 min(const vec3 &v, const vec3 &n) {
    return vec3(std::min(v.x(), n.x()), std::min(v.y(), n.y()), std::min(v.z(), n.z()));
}

inline vec3 max(const vec3 &v, const vec3 &n) {
    return vec3(std::max(v.x(), n.x()), std::max(v.y(), n.y()), std::max(v.z(), n.z()));
}
